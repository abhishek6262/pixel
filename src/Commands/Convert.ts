...
import { DefaultOptions, ImageTypes } from '../Types';
...
export async function convert(
  ...
  if (quality < 1 || quality > 100)
    throw new Error('The quality must be between 1 and 100.');
  ...
  if (statSync(sourcePath).isDirectory()) {
    readdirSync(sourcePath).forEach((file) =>
      convert(
  ...
  try {
    await sharp(sourcePath).toFormat(type, { quality }).toFile(fileName);
  } catch (error) {
    console.error(error);
  }
}
